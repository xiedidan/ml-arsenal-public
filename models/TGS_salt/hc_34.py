...
class Decoder(nn.Module):
    def __init__(self, in_channels, channels, out_channels, shortcut_channels):
        ...
        if shortcut_channels != 0:
            self.bn = nn.BatchNorm2d(shortcut_channels)

    def forward(self, x, e=None):
        # TODO : PixelShuffle_ICNR upsampling
        x = F.upsample(x, scale_factor=2, mode='bilinear', align_corners=True)
        x = F.relu(x, inplace=True)
        #print('x',x.size())
        #print('e',e.size())
        if e is not None:
            # fastai - add activation to upsample and bn to shortcut
            s = self.bn(e)
            x = torch.cat([x,s],1)
...
class Unet_scSE_hyper(nn.Module):
    ...
    def __init__(self):
        ...
        self.center = nn.Sequential(
            ConvBn2d(512,1024,kernel_size=3,padding=1,nonlinearity='relu'),
            nn.ReLU(inplace=True),
            ConvBn2d(1024,512,kernel_size=3,padding=1,nonlinearity='relu'),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2,stride=2),
        )

        self.decoder5 = Decoder(512+512,512,512, 512)
